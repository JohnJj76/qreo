...
class FormOrion extends StatefulWidget {
  const FormOrion({super.key});
  @override
  State<FormOrion> createState() => _FormOrionState();
}

class _FormOrionState extends State<FormOrion> {
  //
  ...
  void initState() {
    ...
    Provider.of<OrionQrProvider>(context, listen: false).mQro.mIdx == null
        ...
        : mFechaController.text =
            Provider.of<OrionQrProvider>(context, listen: false).mQro.mFecha!;
    //
  ...
  _formValidation() async {
    ...
    if (!_formKey.currentState!.validate()) {
      ...
      try {
        if (Provider.of<OrionQrProvider>(context, listen: false).mQro.mIdx ==
            null) {
          // insertar el nuevo qr
          //progressDialogShow(globalContext!);
          await supabase.from('orionqr').insert({
            'fecha': myFechaData,
